 ...
 * que depende de Electron) y pasársela a `main/db/client.ts`, que en sí
 * mismo no sabe nada de Electron — solo recibe rutas ya resueltas. Separa
 * "dónde vive la DB en esta máquina" (aquí) de "cómo se abre y migra" (db/client.ts).
 */

import { join } from 'node:path';
import { app } from 'electron';
import { defaultDbPath, openDatabase, type YCoreDatabase } from '../db/client.js';
